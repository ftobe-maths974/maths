import{l as k}from"./rapidos-visuals-integration.sQBPgUV-.js";import"./editor.CJZspgfY.js";const I="content",N=!0,p=(n,e)=>Math.floor(Math.random()*(e-n+1))+n,f=(n,e)=>{let t;do t=p(n,e);while(t===0);return t};function z(n){const e=n&&n.form||"ax+b=c";if(e==="ax=b"){const s=p(2,7),i=p(2,9);return{...n,equation:`${s}x = ${s*i}`,_seed:Math.random()}}if(e==="x+a=b"){const s=p(1,9),i=p(2,9);return{...n,equation:`x + ${s} = ${s+i}`,_seed:Math.random()}}if(e==="x+a=b-relatif"){let s,i,a;do s=f(-9,9),i=f(-9,9),a=i+s;while(a===0||Math.abs(a)>12);const l=s<0?` - ${-s}`:` + ${s}`;return{...n,equation:`x${l} = ${a}`,_seed:Math.random()}}if(e==="ax=b-relatif"){let s,i;do s=p(-7,7),i=f(-9,9);while(Math.abs(s)<2);const a=s*i,l=s<0?`-${-s}`:`${s}`;return{...n,equation:`${l}x = ${a}`,_seed:Math.random()}}if(e==="ax+b=cx+d"){let s,i,a,l,d;do s=p(3,7),i=p(1,s-1),a=p(2,6),l=p(1,12),d=l-a*(s-i);while(d<=0||d>12);return{...n,equation:`${s}x + ${d} = ${i}x + ${l}`,_seed:Math.random()}}if(e==="ax+b=cx+d-relatif"){let s,i,a,l,d;do s=f(-5,5),i=f(-4,4),a=f(-6,6),l=p(-8,8),d=l-a*(s-i);while(s===i||Math.abs(d)>15);const u=s<0?`-${-s}`:`${s}`,_=i<0?`-${-i}`:`${i}`,y=d<0?` - ${-d}`:` + ${d}`,q=l<0?` - ${-l}`:` + ${l}`;return{...n,equation:`${u}x${y} = ${_}x${q}`,_seed:Math.random()}}const t=p(2,7),r=p(2,7),o=p(1,9),c=t*r+o;return{...n,equation:`${t}x + ${o} = ${c}`,_seed:Math.random()}}const S=`
.ee-wrap {
  padding: 12px 8px;
  margin: 0 auto;
