import{randomize as L}from"./axe-gradue.B5Kfjm3H.js";import"./fullscreen-viewer.qw-yzsCg.js";import"./editor.CJZspgfY.js";const x="content",y=!0,b=`
...
`;let f=!1;function h(){if(f)return;f=!0;const l=document.createElement("style");l.textContent=b,document.head.appendChild(l)}class z extends HTMLElement{connectedCallback(){h(),this.dataset.placeMode="1",this._render()}_render(){this.innerHTML="",this._mode=this.hasAttribute("placeTarget")?"placer":"lire";const e=document.createElement("math974-axe-gradue");for(const a of Array.from(this.attributes))a.name!=="data-place-mode"&&e.setAttribute(a.name,a.value);if(this._mode==="lire"){const a=parseFloat(this.getAttribute("max")||"1"),n=[];for(let p=0;p<=Math.floor(a+1e-9);p++)n.push(p);e.setAttribute("step","1"),e.setAttribute("visibleLabels",JSON.stringify(n))}this._inner=e,this.appendChild(e);const t=this._numFromAttr(),s=this._denFromAttr();if(t===null||s===null)return;const r=document.createElement("details");if(r.className="azf-help",this._mode==="placer"){const a=`<span class="axe-frac"><span class="axe-frac-num">${t}</span><span class="axe-frac-den">${s}</span></span>`;r.innerHTML=`
      ...
      `}else{const a=this._pointLabel(),n=`<span class="axe-frac"><span class="axe-frac-num">${t}</span><span class="axe-frac-den">${s}</span></span>`;r.innerHTML=`
        <summary>🔧 Aide pas-à-pas</summary>
        <div class="azf-body">
          <div class="azf-step step-A azf-step--active" data-phase="identifier_denominateur">
            <span class="azf-step-label">A.</span>
