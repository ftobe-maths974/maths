const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["_astro/rapido-engine.DJuee1ug.js","_astro/rapidos-visuals-integration.C5jZCp8O.js","_astro/editor.CJZspgfY.js","_astro/vis-input.DmqJShOQ.js"])))=>i.map(i=>d[i]);
import{_ as y}from"./editor.CJZspgfY.js";import{ensureSharedStyles as I}from"./vis-input.DmqJShOQ.js";import{parseFormula as v,evaluate as k,parseRef as w,idxToCol as S,formatValue as M,shiftRefs as R,stringify as A,astEqual as z}from"./formula-parser.CuBpGvtX.js";import"./rapido-engine.DJuee1ug.js";import"./rapidos-visuals-integration.C5jZCp8O.js";const G="east",V=!1,L="math974-spreadsheet-css",T=`
math974-spreadsheet {
  display: block;
  font-family: inherit;
  width: 100%;
  container-type: inline-size;
  /* Cellules scale-down auto avec la largeur du composant.
     Min 36px (cible tactile mini), max 80px (confort desktop). */
  --ss-cell-w: clamp(38px, 12cqw, 80px);
  --ss-cell-h: 36px;
  --ss-bord: #cbd5e1;
  --ss-head-bg: #f1f5f9;
  --ss-head-fg: #475569;
...
/* Borne le scaling du .content parent (l'enonce) quand un visuel spreadsheet
   y est monte. Le clamp global atteint 4rem en FS — beaucoup trop pour
   cohabiter avec le tableur en layout 3 colonnes. Permet aussi le wrap. */
...
/* Permet au .content de shrinker sans imposer la largeur du plus long
   mot a toute la grille. */
.q-card .q-card-content {
  min-width: 0;
}
/* Borne le east a 50% de la card. La grille parent fait "auto 1fr auto"
   et le tableur avec container-type:inline-size cree une dependance
   circulaire (parent auto = taille du contenu / contenu = cqw du parent).
   Resultat sans cap : le tableur s'effondre a 0. Le cap 50% force une
   largeur stable depuis laquelle les cqw du tableur sont calcules. */
.q-card.has-visual-east > .q-card-east {
  max-width: 50%;
  min-width: 0;
}
.ss-wrap {
...
`;function B(){if(document.getElementById(L))return;const h=document.createElement("style");h.id=L,h.textContent=T,document.head.appendChild(h)}function C(h,e){if(!h)return e;try{return JSON.parse(h)}catch{return e}}function b(h,e){return S(h)+(e+1)}function N(h){return(h||"").trim()}class F extends HTMLElement{static get observedAttributes(){return["rows","cols","cells","locked","solutions","headers","mode","preset"]}constructor(){super(),this._cells=new Map,this._values=new Map,this._selected=null,this._undo=[],this._activeInput=null,this._currentPhase="cellule_vide",this._mode="formula",this._zonePairs=new Map,this._zoneSolved=new Set,this._activeBadge=null,this._currentTarget=""}connectedCallback(){B(),I(),this.closest(".variant-content")?.classList.add("ss-variant");const e=this.closest(".q-card"),t=this.parentElement,s=this.closest(".q-card-east, .q-card-south, .q-card-north, .q-card-west, .q-content-visual"),r=t?.getBoundingClientRect(),o=e?.getBoundingClientRect();console.log("[ss] mounted",{zone:s?.className,parent:t?.tagName+"."+t?.className,parentW:r?.width,parentH:r?.height,cardW:o?.width,cardH:o?.height,cardClasses:e?.className,hasEast:e?.classList.contains("has-visual-east"),isFS:e?.classList.contains("q-card--fs"),body:document.body.className}),this._render(),this._wireDocLevelListeners()}attributeChangedCallback(){this.isConnected&&this._render()}validate(){if(this._mode==="identify-zones")return this._zoneSolved.size===this._zonePairs.size&&this._zonePairs.size>0;if(this._mode==="cell-click"||this._mode==="range-drag")return this._userSelectionText().toUpperCase()===(this._currentTarget||"").trim().toUpperCase();if(this._mode==="cell-name"||this._mode==="range-name"){const s=(this.querySelector(".ss-name-input")?.value||"").trim().toUpperCase(),r=(this._currentTarget||"").trim().toUpperCase();return s===r&&r!==""}const e=this._targetCells();return e.length===0?!1:e.every(t=>this._isCorrect(t))}toggleSolution(){if(this._mode==="identify-zones"){this._zonePairs.forEach((e,t)=>{const s=this.querySelector(`.ss-zone-badge[data-zone-id="${t}"]`),r=Array.from(this.querySelectorAll(".ss-vocab-pill")).find(o=>o.dataset.label===e);s?.classList.remove("ss-zone-active-sel"),s?.classList.add("ss-zone-ok"),r?.classList.add("ss-pill-used"),r&&(r.disabled=!0),this._zoneSolved.add(t)}),this._updateZoneStatus(),this._updatePhase();return}this._showSolutions=!this._showSolutions,this._renderGrid()}getCurrentPhase(){return this._currentPhase}getCurrentStudentValue(){if(this._mode==="cell-click"||this._mode==="range-drag")return this._userSelectionText();if(this._mode==="cell-name"||this._mode==="range-name")return this.querySelector(".ss-name-input")?.value||"";if(this._activeInput){const e=this._activeInput.dataset.ssRef,t=this._cells.get(e);return t?.raw?t.raw:this._activeInput.value||""}for(const e of this._targetCells()){const t=this._cells.get(e);if(t?.raw&&!this._isCorrect(e))return t.raw}return""}_validateCell(e){if(this._mode==="cell-name"||this._mode==="range-name"){const s=(e.value||"").trim().toUpperCase(),r=(this._currentTarget||"").trim().toUpperCase();return s===r&&r!==""}const t=e.dataset.ssRef;return t?this._isCorrect(t):!1}_readFormula(e){const t=e.dataset.ssRef;return t&&this._cells.get(t)?.raw||""}_readAttrs(){this._rows=Math.max(1,parseInt(this.getAttribute("rows")||"6",10)),this._cols=Math.max(1,parseInt(this.getAttribute("cols")||"5",10));const e=C(this.getAttribute("cells"),{}),t=new Set(C(this.getAttribute("locked"),[])),s=C(this.getAttribute("solutions"),{}),r=C(this.getAttribute("headers"),null);this._declaredMode=this.getAttribute("mode")||"formula",this._mode=this._declaredMode,this._preset=C(this.getAttribute("preset"),null),this._cells.clear();for(const[o,l]of Object.entries(e)){const i={raw:l==null?"":String(l),locked:t.has(o)};this._parseIntoCell(i),this._cells.set(o,i)}for(const o of t)this._cells.has(o)||this._cells.set(o,{raw:"",locked:!0});for(const[o,l]of Object.entries(s)){let n=this._cells.get(o);n||(n={raw:"",locked:!1},this._cells.set(o,n)),n.target=!0,n.sol={formula:l.formula||l,mode:l.mode||"formula",alt:Array.isArray(l.alt)?l.alt:[],fillOnly:!!l.fillOnly}}this._headers=r}_parseIntoCell(e){delete e.ast,delete e.parseError;const t=e.raw;if(typeof t=="string"&&t.trim().startsWith("=")){const s=v(t);s.error?e.parseError=s.error:e.ast=s}}_getCellValue(e,t){const s=b(e,t);this._values.has(s)||this._evaluateCell(s,new Set);const r=this._values.get(s);return r&&typeof r=="object"&&r.error?NaN:r}_evaluateCell(e,t){const s=this._cells.get(e);if(!s){this._values.set(e,"");return}if(s.ast){if(t.has(e)){this._values.set(e,{error:"#CYCLE!"});return}t.add(e);const r={getCell:(o,l)=>{const n=b(o,l);if(t.has(n))throw new q;this._values.has(n)||this._evaluateCell(n,t);const i=this._values.get(n);if(i&&typeof i=="object"&&i.error)throw new q(i.error);return i===void 0?"":i}};try{const o=k(s.ast,r);this._values.set(e,o)}catch(o){this._values.set(e,{error:o.code||"#CYCLE!"})}t.delete(e)}else{const r=s.raw;if(r===""||r==null){this._values.set(e,"");return}const o=parseFloat(String(r).replace(",","."));!isNaN(o)&&String(o)===String(r).replace(",",".").replace(/^\+/,"")?this._values.set(e,o):!isNaN(o)&&/^-?\d+([.,]\d+)?$/.test(String(r).trim())?this._values.set(e,o):this._values.set(e,String(r))}}_recompute(){this._values.clear();for(const e of this._cells.keys())this._values.has(e)||this._evaluateCell(e,new Set)}_render(){this._readAttrs(),this._pickTarget(),this.innerHTML="",this._recompute(),this._buildDom(),this._mode==="identify-zones"?(this._applyPresetSelections(),this._buildIdentifyZones()):this._mode==="cell-name"||this._mode==="range-name"?(this._applyTargetPreset(),this._buildNamingPanel()):(this._mode==="cell-click"||this._mode==="range-drag")&&this._buildClickInstruction(),this._updatePhase(),y(()=>import("./rapido-engine.DJuee1ug.js").then(e=>e.h),__vite__mapDeps([0,1,2])).then(e=>e.wireCardInputs?.(this.closest(".q-card")))}_pickTarget(){const e=Array.isArray(this._preset?.targets)?this._preset.targets:null;e&&e.length>0?this._currentTarget=e[Math.floor(Math.random()*e.length)]:this._currentTarget=(this._preset?.target||"").trim(),this._declaredMode==="cell"?this._mode=Math.random()<.5?"cell-click":"cell-name":this._declaredMode==="range"&&(this._mode=Math.random()<.5?"range-drag":"range-name")}randomize(){this._render()}_applyTargetPreset(){if(!this._tbody||this._preset?.noHighlight)return;const e=this._currentTarget;if(e)if(e.includes(":")){const[t,s]=e.split(":"),r=w(t),o=w(s);if(!r||!o)return;const l=Math.min(r.col,o.col),n=Math.max(r.col,o.col),i=Math.min(r.row,o.row),a=Math.max(r.row,o.row);for(let c=i;c<=a;c++)for(let d=l;d<=n;d++)this._cellElement(d,c)?.classList.add("ss-user-sel")}else{const t=w(e);if(!t)return;this._cellElement(t.col,t.row)?.classList.add("ss-user-sel","ss-user-sel-anchor")}}_buildNamingPanel(){const e=this._currentTarget,t=this._mode==="range-name",s=!!this._preset?.noHighlight,r=document.createElement("div");r.className="ss-vocab-panel";const o=t?"ex. B2:D4":"ex. C5",l=s?`<div class="ss-vocab-panel-title">${this._buildTargetDescription(e,t)}</div>`:"";r.innerHTML=`
    ...
    `,this.appendChild(r)}_buildTargetDescription(e,t){if(!e)return"";if(t){const[r,o]=e.split(":"),l=w(r),n=w(o);if(!l||!n)return`→ Plage : ${e}`;const i=Math.min(l.col,n.col),a=Math.max(l.col,n.col),c=Math.min(l.row,n.row),d=Math.max(l.row,n.row);return`→ Plage du coin <strong>colonne ${S(i)}, ligne ${c+1}</strong> jusqu'au coin <strong>colonne ${S(a)}, ligne ${d+1}</strong>.`}const s=w(e);return s?`→ Colonne <strong>${S(s.col)}</strong>, ligne <strong>${s.row+1}</strong>.`:`→ Cellule : ${e}`}_buildClickInstruction(){const e=this._currentTarget,t=document.createElement("div");t.className="ss-vocab-panel",t.innerHTML=`
