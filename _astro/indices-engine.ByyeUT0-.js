const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["_astro/rapido-engine.CjoVYw_A.js","_astro/rapidos-visuals-integration.C7cjP4uG.js","_astro/editor.CJZspgfY.js","_astro/error-detector.DJe9gvQC.js"])))=>i.map(i=>d[i]);
import{_ as x}from"./editor.CJZspgfY.js";import{T as A,j as E}from"./rapido-engine.CjoVYw_A.js";import"./rapidos-visuals-integration.C7cjP4uG.js";const C=`# Matrice d'indices — fiche 24.04 retour à l'unité (cycle 3)
#
  ...
  satisf-schema-entiers:
    ...
    erreurs_types:
      ...
      - id: dissymetrie_facteurs
        ...
        cible_phase: div_droite|mult_droite
        indice: "Dans un tableau de proportionnalité, **le même facteur** apparaît à gauche et à droite. Vérifie."
`,h=Object.assign({"../data/indices/01-retour-unite.yaml":C,"../data/indices/02-graduer-axe-fraction.yaml":L,"../data/indices/03-tableur.yaml":R,"../data/indices/24.04-retour-unite.yaml":M}),$=(()=>{const e={};for(const n of Object.keys(h)){const a=n.match(/\/([^/]+)\.yaml$/)?.[1];if(a)try{e[a]=E.load(h[n])}catch(t){console.error(`[indices] erreur de parsing ${n}:`,t)}}return e})();function q(e){return e?e.split("/").pop():""}function B(e,n){const a=$[q(e)];return a?.variantes?a.variantes[n]??null:null}function F(e){const n=e?.dataset?.ref;if(!n)return!1;const t=e.querySelector(".variant-content.active")?.dataset?.id;return t?!!B(n,t):!1}function T(e){for(const n of e.querySelectorAll("*"))if(typeof n.getCurrentPhase=="function")return n;return null}async function g(e,n){if(!e)return"";const a=new A;a.reset(),Object.entries(n).forEach(([r,i])=>{(typeof i=="number"||i&&typeof i=="object"&&!Array.isArray(i))&&a.variables.set(r,i)});const t=a.parse(e,"web"),{miniMd:p}=await x(async()=>{const{miniMd:r}=await import("./rapido-engine.CjoVYw_A.js").then(i=>i.h);return{miniMd:r}},__vite__mapDeps([0,1,2]));return p(t)}async function N(e,n={level:1}){const a=e?.dataset?.ref;if(!a)return null;const t=e.querySelector(".variant-content.active"),p=t?.dataset?.id;if(!p)return null;const r=B(a,p),i=T(e),l=i?.getCurrentPhase?.()??"initial";if(l==="done")return{html:"Tu as déjà tout bon ici. Bravo !",phase:l,level:1,isDone:!0};const d=t?.visualData?.config??{},c=Math.min(Math.max(n.level|0,1),3);if(l==="erreur_reponse_directe")try{const{detectErrorPattern:u}=await x(async()=>{const{detectErrorPattern:f}=await import("./error-detector.DJe9gvQC.js");return{detectErrorPattern:f}},__vite__mapDeps([3,0,1,2])),m=i?.querySelector?.(".rapido-input")?.value??"",o=u({ref:q(a),variantId:p,studentValue:m,vars:d});if(o){const b=c===2&&o.relance||o.indice;return{html:await g(b,d),phase:l,level:c,isDone:!1,patternId:o.id}}}catch(u){console.error("[indices] error-detector:",u)}if(r?.patterns_formule&&typeof i?.getCurrentStudentValue=="function")try{const u=i.getCurrentStudentValue()||"";for(const s of r.patterns_formule){if(!s.formula_regex)continue;let m;try{m=new RegExp(s.formula_regex,s.flags??"i")}catch{continue}if(m.test(u)){const f=c===2&&s.relance||s.indice;return{html:await g(f,d),phase:l,level:c,isDone:!1,patternId:s.id}}}}catch(u){console.error("[indices] patterns_formule:",u)}if(!r)return null;const _=r.phases?.[l];if(!_)return null;const v=_[`n${c}`];return v?{html:await g(v,d),phase:l,level:c,isDone:!1}:null}export{F as cardHasIndices,N as getNextIndice};
