import{T as f,j as v}from"./fullscreen-viewer.DKDaLMza.js";import"./editor.CJZspgfY.js";const x=`# ─────────────────────────────────────────────────────────────────────────────
...
#   public/pdf/characterising error types in fraction.pdf
...
`,m=Object.assign({"../../data/error-patterns/01-retour-unite.yaml":x,"../../data/error-patterns/02-graduer-axe-fraction.yaml":g}),h=(()=>{const a={};for(const t of Object.keys(m)){const r=t.match(/\/([^/]+)\.yaml$/)?.[1];if(r)try{a[r]=v.load(m[t])}catch(u){console.error(`[error-patterns] parsing ${t}:`,u)}}return a})();function q(a){if(a==null)return null;const t=String(a).replace(/\s/g,"").replace(",","."),r=parseFloat(t);return isNaN(r)?null:r}function p(a,t,r=.01){return Math.abs(a-t)<r}function C({ref:a,variantId:t,studentValue:r,vars:u}){const s=h[a];if(!s?.patterns||s.variante&&t&&s.variante!==t)return null;const l=q(r);if(l===null)return null;const i=new f;i.reset();for(const[n,e]of Object.entries(u||{}))(typeof e=="number"||e&&typeof e=="object"&&!Array.isArray(e))&&i.variables.set(n,e);const o=u?.a,c=u?.b,d=u?.e;typeof o=="number"&&typeof c=="number"&&typeof d=="number"&&o!==0&&(i.variables.has("answer")||i.variables.set("answer",c/o*d));for(const n of s.patterns)if(n.match)try{const e=i.evaluate(n.match);if(typeof e=="number"&&p(l,e))return n}catch{}for(const n of s.patterns)if(n.match_close)try{const e=i.evaluate(n.match_close);if(typeof e!="number")continue;const b=typeof n.tolerance=="number"?n.tolerance:5;if(!p(l,e)&&Math.abs(l-e)<=b)return n}catch{}return null}export{C as detectErrorPattern};
