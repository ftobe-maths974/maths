import{F as g}from"./template-engine.D16pHiV0.js";import{parseRange as C}from"./utils.MftNdmoG.js";import"./js-yaml.CwjAzRNl.js";const T="north",U=!0,D=!0,M=210,v=165,f=M/2,b=v/2;function R(n){return n*Math.PI/180}function l(n,t){return[f+t*Math.cos(R(n)),b+t*Math.sin(R(n))]}function $(n,t){return`<line x1="${n[0].toFixed(1)}" y1="${n[1].toFixed(1)}"
    x2="${t[0].toFixed(1)}" y2="${t[1].toFixed(1)}"
    stroke="${g.ink}" stroke-width="2" stroke-linecap="round"/>`}function m(n,t,o,e){const[a,s]=l(t,n),[c,r]=l(o,n),i=(o-t+360)%360>180?1:0;return`<path d="M${f},${b} L${a.toFixed(1)},${s.toFixed(1)}
    A${n},${n} 0 ${i} 1 ${c.toFixed(1)},${r.toFixed(1)} Z"
    fill="${e}" fill-opacity="0.25" stroke="${e}" stroke-width="1" stroke-opacity="0.5"/>`}function h(n,t,o,e=11,a="#1e293b"){const[s,c]=l(n,t);return`<text x="${s.toFixed(1)}" y="${c.toFixed(1)}"
    ...
    ${m(s,e+180,e+180+n,y)}
    ${h(e+n/2,s+15,`${n}°`,11,k)}
    ${h(e+180+n/2,s+15,"?",11,y)}`}const k=g.blueDeep,q=g.blueLight,y=g.orange,w=g.muted;function z(n,t,o,e){const s=5+O(e)()*15,c=60,r=20,p=38,i=150+s,u=i-n,d=i-(n+t),E=`${n}°`,L=o==="b"?"?":`${t}°`,j=o==="total"?"?":`${n+t}°`,F=k,S=o==="b"?y:q,_=o==="total"?y:w;let x=$([f,b],l(i,c))+$([f,b],l(u,c))+$([f,b],l(d,c));return x+=h(i,c+14,"A",10)+h(u,c+14,"B",10)+h(d,c+14,"C",10),x+=`<text x="${(f+7).toFixed(1)}" y="${(b+9).toFixed(1)}"
    font-size="9" font-style="italic" fill="#64748b">O</text>`,x+=m(p,d,i,_)+h((i+d)/2,p+15,j,11,_),x+=m(r,u,i,F)+h((i+u)/2,r+15,E,11,F),x+=m(r+6,d,u,S)+h((u+d)/2,r+21,L,11,S),x}function G(n,t){const e=O(t)()>.5,a=65,s=22,c=e?n:180-n,r=180+c,p=180+c/2,i=270+c/2;return`
    ${$(l(180,a),l(0,a))}
    ${$([f,b],l(r,a))}
    ${m(s,180,r,k)}
    ${m(s,r,360,y)}
    ${h(p,s+15,`${c}°`,11,k)}
    ${h(i,s+15,"?",11,y)}`}class I extends HTMLElement{static get observedAttributes(){return["mode","a","b","unknown","seed"]}connectedCallback(){this._render()}attributeChangedCallback(){this.isConnected&&this._render()}_render(){const t=this.getAttribute("mode")||"oppose",o=parseInt(this.getAttribute("a")||"95",10),e=parseInt(this.getAttribute("b")||"40",10),a=this.getAttribute("unknown")||"total",s=this.getAttribute("seed")||"ap";let c="";t==="oppose"?c=B(o,s):t==="adjacent"?c=z(o,e,a,s):t==="supplementaire"&&(c=G(o,s));const r=`<svg viewBox="0 0 ${M} ${v}" width="${M}" height="${v}"
