import{ensureSharedStyles as z}from"./vis-input.CNcQdIym.js";import"./preload-helper.CL8WEfgq.js";import"./rapido-engine.CD5oor2a.js";import"./editor.DFRPxZ68.js";const F="content",P=!0,m={control:"#FFAB19",motion:"#4C97FF"},y=r=>String(r).replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>");function $(r,n,a){const e=s=>`<span class="sp-num">${s}</span>`,t=`<div class="sp-block spz-motion">avancer de ${e(a)} pas</div><div class="sp-block spz-motion">tourner ↻ de ${e(n)} degrés</div>`;return`<div class="sp-prog spz-prog"><div class="sp-block spz-hat">quand 🏁 cliqué</div><div class="sp-loop"><div class="sp-loop-top">répéter ${e(r)} fois</div><div class="sp-loop-mid"><div class="sp-loop-bar"></div><div class="sp-loop-body">${t}</div></div><div class="sp-loop-bot"></div></div></div>`}function k(r){const t=[];for(let s=0;s<r;s++){const l=-Math.PI/2+2*Math.PI*s/r;t.push(`${(46+38*Math.cos(l)).toFixed(1)},${(46+38*Math.sin(l)).toFixed(1)}`)}return`<svg viewBox="0 0 92 92" width="92" height="92" aria-label="polygone régulier à ${r} côtés">
    <polygon points="${t.join(" ")}" fill="none" stroke="#0369a1" stroke-width="3" stroke-linejoin="round"/></svg>`}function w(r,n,a){let e=0,t=0,s=-90;const l=[[e,t]];for(let o=0;o<r;o++){const p=s*Math.PI/180;e+=a*Math.cos(p),t+=a*Math.sin(p),l.push([e,t]),s+=n}return l}function M(r,n){const a=r.map(d=>d[0]),e=r.map(d=>d[1]),t=Math.min(...a),s=Math.max(...a),l=Math.min(...e),o=Math.max(...e),p=10,u=s-t||1,h=o-l||1,g=`${t-p} ${l-p} ${u+2*p} ${h+2*p}`,i="M "+r.map(d=>`${d[0].toFixed(1)},${d[1].toFixed(1)}`).join(" L "),c=r[r.length-1],f=r[0],x=Math.hypot(c[0]-f[0],c[1]-f[1])<1;return`<svg viewBox="${g}" width="120" height="120" style="max-width:120px" aria-label="tracé du programme">
...
`;function _(){if(document.getElementById("spz-styles"))return;const r=document.createElement("style");r.id="spz-styles",r.textContent=S,document.head.appendChild(r)}let q=0;class E extends HTMLElement{static get observedAttributes(){return["n","ang","side","options","correct"]}connectedCallback(){z(),_(),this.dataset.placeMode="1",this._render()}attributeChangedCallback(){this.isConnected&&this._render()}_cfg(){const n=parseInt(this.getAttribute("n"),10),a=parseInt(this.getAttribute("ang"),10),e=parseInt(this.getAttribute("side"),10);let t=[];try{t=JSON.parse(this.getAttribute("options")||"[]")}catch{}const s=parseInt(this.getAttribute("correct"),10);return{n,ang:a,side:e,options:t,correct:s}}_render(){const{n,side:a,options:e,correct:t}=this._cfg();if(!Number.isFinite(n)||!e.length){this.innerHTML="";return}this.innerHTML="";const s=b[n]||`polygone à ${n} côtés`,l=`spz-${++q}`,o=document.createElement("div");o.className="spz-wrap",o.innerHTML=`
      <div class="spz-q">${k(n)}<span>Quel programme dessine ce <strong>${s}</strong>&nbsp;?</span></div>
      ...
      <div class="spz-result" aria-live="polite"></div>`,this.appendChild(o),this._wrap=o,this._resultEl=o.querySelector(".spz-result"),o.querySelectorAll("input[type=radio]").forEach(p=>{p.addEventListener("change",()=>this._validate(!0))})}_validate(n){const{correct:a}=this._cfg(),e=this._wrap?.querySelector("input[type=radio]:checked");if(this._wrap?.querySelectorAll(".spz-opt").forEach(f=>{f.classList.remove("spz-correct","spz-incorrect"),f.querySelector(".spz-opt-fb").textContent=""}),!e)return;const t=parseInt(e.value,10),s=t===a,l=this._wrap.querySelector(`.spz-opt[data-i="${t}"]`);l.classList.add(s?"spz-correct":"spz-incorrect"),l.querySelector(".spz-opt-fb").textContent=s?"😀":"😞";const{n:o,ang:p,side:u,options:h}=this._cfg(),g=h[t],i=w(g.rep,g.ang,u);let c;return s?c=`😀 Le tracé se **referme** : c'est bien le ${b[o]||"polygone"} !`:g.rep!==o&&g.ang===p?c=`😞 Mauvais nombre de répétitions : il faut **répéter ${o} fois** (un par côté), pas ${g.rep}.`:g.rep===o&&g.ang!==p?c=`😞 Le tracé ne se referme pas. À chaque sommet, le lutin doit tourner de **360 ÷ ${o} = ${p}°**, pas ${g.ang}°.`:c=`😞 Ni le bon angle ni le bon nombre de tours : ${o} côtés → **répéter ${o}**, **tourner ${p}°**.`,this._resultEl&&(this._resultEl.innerHTML=`<div class="spz-result-in spz-${s?"ok":"ko"}">`+M(i,s)+`<span class="spz-result-msg">${y(c)}</span></div>`),!s&&n&&this.closest(".q-card")?.dispatchEvent(new CustomEvent("zefor-indice-auto-pop",{detail:{studentValue:String(t),source:this},bubbles:!0})),s}getCurrentPhase(){const n=this._wrap?.querySelector("input[type=radio]:checked");if(!n)return"reponse_directe";const{correct:a}=this._cfg();return parseInt(n.value,10)===a?"done":"erreur_reponse_directe"}validate(){return this._validate(!1)}toggleSolution(n){if(!n||!this._wrap)return;const{correct:a}=this._cfg(),e=this._wrap.querySelector(`.spz-opt[data-i="${a}"] input`);e&&(e.checked=!0,this._validate(!1))}}customElements.get("math974-scratch-poly-zefor")||customElements.define("math974-scratch-poly-zefor",E);function j(r,n,a){const e=i=>{const c=String(i).trim(),f=c.match(/^(\d+)\.\.(\d+)$/);if(f){const d=+f[1],v=+f[2];return d+Math.floor(Math.random()*(v-d+1))}const x=c.split(",").map(d=>+d.trim()).filter(d=>!isNaN(d));return x[Math.floor(Math.random()*x.length)]},t=e(n?.n??"3..10"),s=e(n?.side??"40,50,60,80,100"),l=Math.round(360/t),o={rep:t,ang:l},p=[{rep:t,ang:t},{rep:t,ang:180-l},{rep:t+(Math.random()<.5?1:-1),ang:l},{rep:t,ang:Math.round(360/(t+1))}].filter(i=>!(i.rep===o.rep&&i.ang===o.ang)&&i.rep>=3&&i.ang>0),u=[];for(const i of p){if(u.length>=2)break;u.some(c=>c.rep===i.rep&&c.ang===i.ang)||u.push(i)}const h=[o,...u];for(let i=h.length-1;i>0;i--){const c=Math.floor(Math.random()*(i+1));[h[i],h[c]]=[h[c],h[i]]}const g=h.findIndex(i=>i.rep===o.rep&&i.ang===o.ang);return{...r,n:t,ang:l,side:s,options:h,correct:g}}export{P as autoScale,F as defaultPosition,j as randomize};
