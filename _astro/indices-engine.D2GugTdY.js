const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["_astro/rapido-engine.DdpoK3Ts.js","_astro/rapidos-visuals-integration.BuhAApdn.js","_astro/editor.CJZspgfY.js","_astro/error-detector.-LqzW3sX.js"])))=>i.map(i=>d[i]);
import{_ as x}from"./editor.CJZspgfY.js";import{T as A,j as E}from"./rapido-engine.DdpoK3Ts.js";import"./rapidos-visuals-integration.BuhAApdn.js";const L=`# Matrice d'indices — fiche 24.04 retour à l'unité (cycle 3)
#
  ...
  satisf-schema-entiers:
    ...
    erreurs_types:
      ...
      - id: dissymetrie_facteurs
        ...
        cible_phase: div_droite|mult_droite
        indice: "Dans un tableau de proportionnalité, **le même facteur** apparaît à gauche et à droite. Vérifie."
`,h=Object.assign({"../data/indices/01-retour-unite.yaml":L,"../data/indices/02-graduer-axe-fraction.yaml":C,"../data/indices/04-tableur-dnb.yaml":M,"../data/indices/24.04-retour-unite.yaml":y}),$=(()=>{const e={};for(const n of Object.keys(h)){const a=n.match(/\/([^/]+)\.yaml$/)?.[1];if(a)try{e[a]=E.load(h[n])}catch(t){console.error(`[indices] erreur de parsing ${n}:`,t)}}return e})();function B(e){return e?e.split("/").pop():""}function q(e,n){const a=$[B(e)];return a?.variantes?a.variantes[n]??null:null}function N(e){const n=e?.dataset?.ref;if(!n)return!1;const t=e.querySelector(".variant-content.active")?.dataset?.id;return t?!!q(n,t):!1}function T(e){for(const n of e.querySelectorAll("*"))if(typeof n.getCurrentPhase=="function")return n;return null}async function _(e,n){if(!e)return"";const a=new A;a.reset(),Object.entries(n).forEach(([r,i])=>{(typeof i=="number"||i&&typeof i=="object"&&!Array.isArray(i))&&a.variables.set(r,i)});const t=a.parse(e,"web"),{miniMd:p}=await x(async()=>{const{miniMd:r}=await import("./rapido-engine.DdpoK3Ts.js").then(i=>i.f);return{miniMd:r}},__vite__mapDeps([0,1,2]));return p(t)}async function V(e,n={level:1}){const a=e?.dataset?.ref;if(!a)return null;const t=e.querySelector(".variant-content.active"),p=t?.dataset?.id;if(!p)return null;const r=q(a,p),i=T(e),u=i?.getCurrentPhase?.()??"initial";if(u==="done")return{html:"Tu as déjà tout bon ici. Bravo !",phase:u,level:1,isDone:!0};const d=t?.visualData?.config??{},c=Math.min(Math.max(n.level|0,1),3);if(u==="erreur_reponse_directe")try{const{detectErrorPattern:l}=await x(async()=>{const{detectErrorPattern:f}=await import("./error-detector.-LqzW3sX.js");return{detectErrorPattern:f}},__vite__mapDeps([3,0,1,2])),m=i?.querySelector?.(".rapido-input")?.value??"",o=l({ref:B(a),variantId:p,studentValue:m,vars:d});if(o){const b=c===2&&o.relance||o.indice;return{html:await _(b,d),phase:u,level:c,isDone:!1,patternId:o.id}}}catch(l){console.error("[indices] error-detector:",l)}if(r?.patterns_formule&&typeof i?.getCurrentStudentValue=="function")try{const l=i.getCurrentStudentValue()||"";for(const s of r.patterns_formule){if(!s.formula_regex)continue;let m;try{m=new RegExp(s.formula_regex,s.flags??"i")}catch{continue}if(m.test(l)){const f=c===2&&s.relance||s.indice;return{html:await _(f,d),phase:u,level:c,isDone:!1,patternId:s.id}}}}catch(l){console.error("[indices] patterns_formule:",l)}if(!r)return null;const g=r.phases?.[u];if(!g)return null;const v=g[`n${c}`];return v?{html:await _(v,d),phase:u,level:c,isDone:!1}:null}export{N as cardHasIndices,V as getNextIndice};
