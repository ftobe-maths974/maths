const L="north",Y=!0;function A(n){return n*Math.PI/180}function l(n,t){return[105+t*Math.cos(A(n)),82.5+t*Math.sin(A(n))]}function f(n,t){return`<line x1="${n[0].toFixed(1)}" y1="${n[1].toFixed(1)}"
    ...
    font-size="${e}" font-weight="700" fill="${a}">${o}</text>`}function y(n){let t=0;for(const o of String(n))t=Math.imul(t,31)+o.charCodeAt(0)|0;return t=t>>>0||1,()=>(t=Math.imul(1664525,t)+1013904223>>>0,t/4294967296)}function S(n,t){const e=10+y(t)()*25,a=65,s=20;return`
    ${f(l(e,a),l(e+180,a))}
    ${f(l(e+n,a),l(e+n+180,a))}
    ${b(s,e,e+n,m)}
    ${b(s,e+180,e+180+n,x)}
    ${p(e+n/2,s+15,`${n}°`,11,m)}
    ${p(e+180+n/2,s+15,"?",11,x)}`}const m="#1d4ed8",_="#3b82f6",x="#ea580c",F="#64748b";function E(n,t,o,e){const s=5+y(e)()*15,c=60,r=20,u=38,i=150+s,h=i-n,d=i-(n+t),M=`${n}°`,v=o==="b"?"?":`${t}°`,O=o==="total"?"?":`${n+t}°`,C=m,k=o==="b"?x:_,g=o==="total"?x:F;let $=f([105,82.5],l(i,c))+f([105,82.5],l(h,c))+f([105,82.5],l(d,c));return $+=p(i,c+14,"A",10)+p(h,c+14,"B",10)+p(d,c+14,"C",10),$+=`<text x="${112 .toFixed(1)}" y="${(82.5+9).toFixed(1)}"
    ...
    ${b(s,r,360,x)}
    ${p(u,s+15,`${c}°`,11,m)}
    ${p(i,s+15,"?",11,x)}`}class B extends HTMLElement{static get observedAttributes(){return["mode","a","b","unknown","seed"]}connectedCallback(){this._render()}attributeChangedCallback(){this.isConnected&&this._render()}_render(){const t=this.getAttribute("mode")||"oppose",o=parseInt(this.getAttribute("a")||"95",10),e=parseInt(this.getAttribute("b")||"40",10),a=this.getAttribute("unknown")||"total",s=this.getAttribute("seed")||"ap";let c="";t==="oppose"?c=S(o,s):t==="adjacent"?c=E(o,e,a,s):t==="supplementaire"&&(c=R(o,s));const r=`<svg viewBox="0 0 210 165" width="210" height="165"
      style="display:block;flex-shrink:0;width:210px;height:165px;">${c}</svg>`,u=t==="supplementaire"?["supplementaire","adjacent"]:[t],h=[{key:"adjacent",label:"Adjacents"},{key:"oppose",label:"Opposés par le sommet"},{key:"supplementaire",label:"Supplémentaires"}].map(d=>`
