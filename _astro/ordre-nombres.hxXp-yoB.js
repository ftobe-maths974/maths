...
`;function z(){if(document.getElementById("on-styles"))return;const s=document.createElement("style");s.id="on-styles",s.textContent=_,document.head.appendChild(s)}function y(s,t){return Math.round(s/t)*t}function w(s,t){if(!t||t>=1)return String(Math.round(s));const e=Math.max(0,Math.round(-Math.log10(t)));return s.toFixed(e).replace(".",",")}function C(s){const t=[...s];for(let e=t.length-1;e>0;e--){const o=Math.floor(Math.random()*(e+1));[t[e],t[o]]=[t[o],t[e]]}return t}function $(s,t){return[...s].sort((e,o)=>t==="asc"?e.value-o.value:o.value-e.value)}class L extends HTMLElement{static get observedAttributes(){return["items","direction","unit","step","layout"]}connectedCallback(){z(),this._solutionVisible=!1,this._locked=!1,this._render()}attributeChangedCallback(){this.isConnected&&(this._solutionVisible=!1,this._locked=!1,this._render())}_cfg(){return{items:JSON.parse(this.getAttribute("items")||"[]"),direction:this.getAttribute("direction")||"desc",unit:this.getAttribute("unit")||"",step:parseFloat(this.getAttribute("step")||"1"),layout:this.getAttribute("layout")||"vertical"}}_itemHtml(t,e,o){const n=t.label?`<span class="on-name">${t.label}</span>`:"",r=`<span class="on-val">${t.display??w(t.value,e)}${o?" "+o:""}</span>`;return`<span class="on-handle">⠿</span>${n}${r}`}_render(){const{items:t,direction:e,unit:o,step:n,layout:r}=this._cfg();if(!t.length){this.innerHTML="";return}const d=e==="random"?Math.random()<.5?"asc":"desc":e||"desc";this._direction=d;const l=$(t,d);this.dataset.solution=l.map(a=>a.short??a.label).join(","),this.dataset.placeMode="1";let i=C(t);i.map(a=>a.short??a.label).join(",")===this.dataset.solution&&i.length>1&&([i[0],i[1]]=[i[1],i[0]]);const g=`Classer dans l'ordre ${d==="asc"?"croissant":"décroissant"} <span style="font-weight:400">(${r==="horizontal"?d==="asc"?"du plus petit au plus grand":"du plus grand au plus petit":d==="asc"?"du plus petit en haut, au plus grand en bas":"du plus grand en haut, au plus petit en bas"})</span>`,c=r==="horizontal"?"on-list horiz":"on-list";this.innerHTML=`
      <div class="on-dir">${g}</div>
      <div class="${c}">
        ${i.map(a=>`<div class="on-item" data-short="${a.short??a.label}" data-chevron="${d==="asc"?"<":">"}">${this._itemHtml(a,n,o)}</div>`).join("")}
      </div>
    `,this._setupDrag(r)}validate(){const{items:t}=this._cfg(),e=$(t,this._direction||"desc").map(o=>o.short??o.label);return this.querySelectorAll(".on-item").forEach((o,n)=>{o.classList.toggle("on-correct",o.dataset.short===e[n]),o.classList.toggle("on-incorrect",o.dataset.short!==e[n])}),[...this.querySelectorAll(".on-item")].every(o=>o.classList.contains("on-correct"))}toggleSolution(){this._solutionVisible=!this._solutionVisible;const t=this.querySelector(".on-list"),{items:e}=this._cfg(),o=this._direction||"desc";if(this._solutionVisible){this._studentOrder=[...t.querySelectorAll(".on-item")];const n=$(e,o),r=[...t.querySelectorAll(".on-item")];n.forEach(d=>{const l=r.find(i=>i.dataset.short===(d.short??d.label));l&&(t.appendChild(l),l.classList.add("on-correct"),l.classList.remove("on-incorrect"))}),this._locked=!0}else(this._studentOrder||[]).forEach(n=>{t.appendChild(n),n.classList.remove("on-correct","on-incorrect")}),this._studentOrder=null,this._locked=!1;return this._solutionVisible}_setupDrag(t){const e=this.querySelector(".on-list"),o=t==="horizontal";let n=null,r=null,d=null,l=0,i=0,p=0,f=0;e.addEventListener("pointerdown",c=>{if(this._locked||r)return;const a=c.target.closest(".on-item");if(!a)return;c.preventDefault();const h=a.getBoundingClientRect(),m=this.getBoundingClientRect(),u=this.offsetWidth&&m.width?m.width/this.offsetWidth:1;l=c.clientX,i=c.clientY,p=h.left,f=h.top,d=c.pointerId,n=document.createElement("div"),n.className="on-ghost",n.innerHTML=a.innerHTML,n.style.top=h.top+"px",n.style.left=h.left+"px",n.style.width=h.width/u+"px",n.style.height=h.height/u+"px",u!==1&&(n.style.transformOrigin="top left",n.style.transform=`scale(${u})`),o&&(n.style.flexDirection="column",n.style.alignItems="center",n.style.textAlign="center"),document.body.appendChild(n),a.style.opacity="0.35",e.classList.add("dragging"),r=a,e.setPointerCapture(d)}),e.addEventListener("pointermove",c=>{if(!r||c.pointerId!==d)return;c.preventDefault(),n.style.top=f+c.clientY-i+"px",n.style.left=p+c.clientX-l+"px";const a=[...e.querySelectorAll(".on-item")].filter(m=>m!==r);let h=!1;for(const m of a){const u=m.getBoundingClientRect(),x=o?u.left+u.width/2:u.top+u.height/2;if((o?c.clientX:c.clientY)<x){e.insertBefore(r,m),h=!0;break}}h||e.appendChild(r)});const g=c=>{!r||c.pointerId!==d||(r.style.opacity="",e.classList.remove("dragging"),n.remove(),n=null,r=null,d=null)};e.addEventListener("pointerup",g),e.addEventListener("pointercancel",g)}}customElements.get("math974-ordre-nombres")||customElements.define("math974-ordre-nombres",L);const S="north",E=!0;function k(s){const t=["A","B","C","D"],e=[...Array(s.length).keys()];for(let o=e.length-1;o>0;o--){const n=Math.floor(Math.random()*(o+1));[e[o],e[n]]=[e[n],e[o]]}return s.map((o,n)=>({...o,short:t[e[n]]}))}function A(s,t,e){const[o,n]=t.baseRange??[3,7],[r,d]=t.dixRange??[1,7],l=o+Math.floor(Math.random()*(n-o+1)),i=r+Math.floor(Math.random()*(d-r+1)),p=1+Math.floor(Math.random()*8),f=2+Math.floor(Math.random()*7),g=Math.random()<.5?1:-1,c=Math.random()<.5,a=Math.random()<.5,h=l*1e3+i*100+p*10,m=(h+f)/1e3,u=(h+f+g)/1e3,x=c?(h+10)/1e3:h/1e3,v=a?(l*1e3+(i+1)*100)/1e3:(l*1e3+i*100)/1e3,b=[{value:m,display:`${l},${i}${p}${f}`},{value:u,display:`${l},${i}${p}${f+g}`},{value:x,display:c?`${l},${i}${p+1}`:`${l},${i}${p}`},{value:v,display:a?`${l},${i+1}`:`${l},${i}`}];return{...s,items:k(b),direction:e,layout:t.layout??"horizontal",unit:t.unit??""}}function D(s,t,e){const o=t?.direction??e?.direction??s.direction??"random",n=o==="random"?Math.random()<.5?"asc":"desc":o;if(t?.trapType==="centieme"||t?.trapType==="millieme")return A(s,t,n);if(!t||!t.medianRange)return{...s,direction:n};const r=t.step??s.step??1,d=t.unit??s.unit??"",l=t.layout??s.layout??"vertical",[i,p]=t.medianRange??[200,800],f=y(i+Math.random()*(p-i),r),[g,c]=t.deltaRange??[20,100],a=Math.max(r,y(g+Math.random()*(c-g),r));let m=[-1.5,-.5,.5,1.5].map(b=>y(f+b*a,r));const u=Math.min(...m);if(u<=0){const b=y(-u+r,r);m=m.map(M=>y(M+b,r))}const x=["A","B","C","D"],v=m.map((b,M)=>({short:x[M],label:"",value:b}));return{...s,items:v,direction:n,unit:d,step:r,layout:l}}export{E as autoScale,L as default,S as defaultPosition,D as randomize};
