const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["_astro/rapido-engine.7ZIDQB1d.js","_astro/rapidos-visuals-integration.DGD9e6nZ.js","_astro/editor.CJZspgfY.js","_astro/error-detector.CM9Jc8FB.js"])))=>i.map(i=>d[i]);
import{_ as x}from"./editor.CJZspgfY.js";import{T as A,j as E}from"./rapido-engine.7ZIDQB1d.js";import"./rapidos-visuals-integration.DGD9e6nZ.js";const L=`# Matrice d'indices — fiche 24.04 retour à l'unité (cycle 3)
#
  ...
  satisf-schema-entiers:
    ...
    erreurs_types:
      ...
      - id: dissymetrie_facteurs
        ...
        cible_phase: div_droite|mult_droite
        indice: "Dans un tableau de proportionnalité, **le même facteur** apparaît à gauche et à droite. Vérifie."
`,h=Object.assign({"../data/indices/01-retour-unite.yaml":L,"../data/indices/02-graduer-axe-fraction.yaml":C,"../data/indices/03-tableur-prop.yaml":M,"../data/indices/04-tableur-dnb.yaml":$,"../data/indices/24.04-retour-unite.yaml":y}),T=(()=>{const e={};for(const n of Object.keys(h)){const a=n.match(/\/([^/]+)\.yaml$/)?.[1];if(a)try{e[a]=E.load(h[n])}catch(t){console.error(`[indices] erreur de parsing ${n}:`,t)}}return e})();function q(e){return e?e.split("/").pop():""}function B(e,n){const a=T[q(e)];return a?.variantes?a.variantes[n]??null:null}function j(e){const n=e?.dataset?.ref;if(!n)return!1;const t=e.querySelector(".variant-content.active")?.dataset?.id;return t?!!B(n,t):!1}function S(e){for(const n of e.querySelectorAll("*"))if(typeof n.getCurrentPhase=="function")return n;return null}async function _(e,n){if(!e)return"";const a=new A;a.reset(),Object.entries(n).forEach(([r,i])=>{(typeof i=="number"||i&&typeof i=="object"&&!Array.isArray(i))&&a.variables.set(r,i)});const t=a.parse(e,"web"),{miniMd:p}=await x(async()=>{const{miniMd:r}=await import("./rapido-engine.7ZIDQB1d.js").then(i=>i.f);return{miniMd:r}},__vite__mapDeps([0,1,2]));return p(t)}async function V(e,n={level:1}){const a=e?.dataset?.ref;if(!a)return null;const t=e.querySelector(".variant-content.active"),p=t?.dataset?.id;if(!p)return null;const r=B(a,p),i=S(e),s=i?.getCurrentPhase?.()??"initial";if(s==="done")return{html:"Tu as déjà tout bon ici. Bravo !",phase:s,level:1,isDone:!0};const d=t?.visualData?.config??{},c=Math.min(Math.max(n.level|0,1),3);if(s==="erreur_reponse_directe")try{const{detectErrorPattern:u}=await x(async()=>{const{detectErrorPattern:f}=await import("./error-detector.CM9Jc8FB.js");return{detectErrorPattern:f}},__vite__mapDeps([3,0,1,2])),m=i?.querySelector?.(".rapido-input")?.value??"",o=u({ref:q(a),variantId:p,studentValue:m,vars:d});if(o){const b=c===2&&o.relance||o.indice;return{html:await _(b,d),phase:s,level:c,isDone:!1,patternId:o.id}}}catch(u){console.error("[indices] error-detector:",u)}if(r?.patterns_formule&&typeof i?.getCurrentStudentValue=="function")try{const u=i.getCurrentStudentValue()||"";for(const l of r.patterns_formule){if(!l.formula_regex)continue;let m;try{m=new RegExp(l.formula_regex)}catch{continue}if(m.test(u)){const f=c===2&&l.relance||l.indice;return{html:await _(f,d),phase:s,level:c,isDone:!1,patternId:l.id}}}}catch(u){console.error("[indices] patterns_formule:",u)}if(!r)return null;const v=r.phases?.[s];if(!v)return null;const g=v[`n${c}`];return g?{html:await _(g,d),phase:s,level:c,isDone:!1}:null}export{j as cardHasIndices,V as getNextIndice};
