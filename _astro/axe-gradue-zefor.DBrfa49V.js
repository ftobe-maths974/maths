import{r as u}from"./fullscreen-viewer.B97TysPw.js";import{randomize as $}from"./axe-gradue.UOJWWXXm.js";import"./editor.CJZspgfY.js";const y="content",_=!0,b=`
...
`;let m=!1;function z(){if(m)return;m=!0;const d=document.createElement("style");d.textContent=b,document.head.appendChild(d)}class g extends HTMLElement{connectedCallback(){z(),this.hasAttribute("placeTarget")?this.dataset.placeMode="1":delete this.dataset.placeMode,this._render()}_render(){this.innerHTML="",this._mode=this.hasAttribute("placeTarget")?"placer":"lire";const e=document.createElement("math974-axe-gradue");for(const a of Array.from(this.attributes))a.name!=="data-place-mode"&&e.setAttribute(a.name,a.value);const l=parseFloat(this.getAttribute("max")||"1"),p=[];for(let a=0;a<=Math.floor(l+1e-9);a++)p.push(a);e.setAttribute("step","1"),e.setAttribute("visibleLabels",JSON.stringify(p)),this._inner=e,this.appendChild(e);const t=this._numFromAttr(),r=this._denFromAttr();if(t===null||r===null)return;const s=document.createElement("details");s.className="azf-help";const i=`<span class="axe-frac"><span class="axe-frac-num">${t}</span><span class="axe-frac-den">${r}</span></span>`,n=(a,h)=>`<span class="azf-frac-inline"><span class="azf-frac-inline-num"><input class="azf-step-inp" data-expected="${a}" type="text" inputmode="numeric" size="2" autocomplete="off" placeholder="?"></span><span class="azf-frac-inline-bar"></span><span class="azf-frac-inline-den"><input class="azf-step-inp" data-expected="${h}" type="text" inputmode="numeric" size="2" autocomplete="off" placeholder="?"></span></span>`,o=`
