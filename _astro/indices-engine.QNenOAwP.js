const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["_astro/rapido-engine.BchZ4LyO.js","_astro/editor.CJZspgfY.js","_astro/rapidos-visuals-integration.CImjkDbj.js","_astro/js-yaml.CwjAzRNl.js","_astro/error-detector.47xtZ8ou.js"])))=>i.map(i=>d[i]);
import{_ as x}from"./editor.CJZspgfY.js";import $ from"./js-yaml.CwjAzRNl.js";import{T as A}from"./rapido-engine.BchZ4LyO.js";import"./rapidos-visuals-integration.CImjkDbj.js";const E=`# Matrice d'indices — fiche 24.04 retour à l'unité (cycle 3)
#
...
`,L=`# Matrice d'indices — Zefor 02-glisse-nombre (×/÷ 10 ; 100 ; 1000)
#
# BROUILLON v0 — à relire/corriger par Florian (style, formulation, pédago).
#
...
`,y=`# Matrice d'indices — Zefor 03-volume-cubes (calcul de volume, cubes de 1 cm³)
#
...
variantes:
  ...
  satisf-pave: *vc
  expert-creuse: *vc
`,M=`# Matrice d'indices — fiche 24.04 retour à l'unité (cycle 3)
#
# BROUILLON v0 — à relire/corriger par Florian (style, formulation, pédago).
#
# Format :
  ...
  satisf-schema-entiers:
    ...
    erreurs_types:
      ...
      - id: dissymetrie_facteurs
        ...
        cible_phase: div_droite|mult_droite
        indice: "Dans un tableau de proportionnalité, **le même facteur** apparaît à gauche et à droite. Vérifie."
`,h=Object.assign({"../data/indices/01-retour-unite.yaml":E,"../data/indices/02-glisse-nombre.yaml":L,"../data/indices/02-graduer-axe-fraction.yaml":C,"../data/indices/03-tableur.yaml":R,"../data/indices/03-volume-cubes.yaml":y,"../data/indices/24.04-retour-unite.yaml":M}),T=(()=>{const e={};for(const n of Object.keys(h)){const a=n.match(/\/([^/]+)\.yaml$/)?.[1];if(a)try{e[a]=$.load(h[n])}catch(r){console.error(`[indices] erreur de parsing ${n}:`,r)}}return e})();function q(e){return e?e.split("/").pop():""}function B(e,n){const a=T[q(e)];return a?.variantes?a.variantes[n]??null:null}function I(e){const n=e?.dataset?.ref;if(!n)return!1;const r=e.querySelector(".variant-content.active")?.dataset?.id;return r?!!B(n,r):!1}function P(e){for(const n of e.querySelectorAll("*"))if(typeof n.getCurrentPhase=="function")return n;return null}async function g(e,n){if(!e)return"";const a=new A;a.reset(),Object.entries(n).forEach(([t,i])=>{(typeof i=="number"||i&&typeof i=="object"&&!Array.isArray(i))&&a.variables.set(t,i)});const r=a.parse(e,"web"),{miniMd:p}=await x(async()=>{const{miniMd:t}=await import("./rapido-engine.BchZ4LyO.js").then(i=>i.j);return{miniMd:t}},__vite__mapDeps([0,1,2,3]));return p(r)}async function j(e,n={level:1}){const a=e?.dataset?.ref;if(!a)return null;const r=e.querySelector(".variant-content.active"),p=r?.dataset?.id;if(!p)return null;const t=B(a,p),i=P(e),l=i?.getCurrentPhase?.()??"initial";if(l==="done")return{html:"Tu as déjà tout bon ici. Bravo !",phase:l,level:1,isDone:!0};const d=r?.visualData?.config??{},o=Math.min(Math.max(n.level|0,1),3);if(l==="erreur_reponse_directe")try{const{detectErrorPattern:u}=await x(async()=>{const{detectErrorPattern:f}=await import("./error-detector.47xtZ8ou.js");return{detectErrorPattern:f}},__vite__mapDeps([4,3,0,1,2])),m=i?.querySelector?.(".rapido-input")?.value??"",c=u({ref:q(a),variantId:p,studentValue:m,vars:d});if(c){const b=o===2&&c.relance||c.indice;return{html:await g(b,d),phase:l,level:o,isDone:!1,patternId:c.id}}}catch(u){console.error("[indices] error-detector:",u)}if(t?.patterns_formule&&typeof i?.getCurrentStudentValue=="function")try{const u=i.getCurrentStudentValue()||"";for(const s of t.patterns_formule){if(!s.formula_regex)continue;let m;try{m=new RegExp(s.formula_regex,s.flags??"i")}catch{continue}if(m.test(u)){const f=o===2&&s.relance||s.indice;return{html:await g(f,d),phase:l,level:o,isDone:!1,patternId:s.id}}}}catch(u){console.error("[indices] patterns_formule:",u)}if(!t)return null;const v=t.phases?.[l];if(!v)return null;const _=v[`n${o}`];return _?{html:await g(_,d),phase:l,level:o,isDone:!1}:null}export{I as cardHasIndices,j as getNextIndice};
