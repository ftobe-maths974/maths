import g from"./js-yaml.CwjAzRNl.js";import{T as v}from"./rapido-engine.-bjle9Rg.js";import"./editor.CJZspgfY.js";import"./rapidos-visuals-integration.C0HxzeXt.js";const b=`# ─────────────────────────────────────────────────────────────────────────────
...
`,m=Object.assign({"../../data/error-patterns/01-retour-unite.yaml":b,"../../data/error-patterns/02-glisse-nombre.yaml":x,"../../data/error-patterns/02-graduer-axe-fraction.yaml":h}),_=(()=>{const r={};for(const a of Object.keys(m)){const i=a.match(/\/([^/]+)\.yaml$/)?.[1];if(i)try{r[i]=g.load(m[a])}catch(s){console.error(`[error-patterns] parsing ${a}:`,s)}}return r})();function q(r){if(r==null)return null;const a=String(r).replace(/\s/g,"").replace(",","."),i=parseFloat(a);return isNaN(i)?null:i}function p(r,a,i=.01){return Math.abs(r-a)<i}function E({ref:r,variantId:a,studentValue:i,vars:s}){const u=_[r];if(!u?.patterns||u.variante&&a&&u.variante!==a)return null;const l=q(i);if(l===null)return null;const t=new v;t.reset();for(const[n,e]of Object.entries(s||{}))(typeof e=="number"||e&&typeof e=="object"&&!Array.isArray(e))&&t.variables.set(n,e);const o=s?.a,c=s?.b,d=s?.e;typeof o=="number"&&typeof c=="number"&&typeof d=="number"&&o!==0&&(t.variables.has("answer")||t.variables.set("answer",c/o*d));for(const n of u.patterns)if(n.match)try{const e=t.evaluate(n.match);if(typeof e=="number"&&p(l,e))return n}catch{}for(const n of u.patterns)if(n.match_close)try{const e=t.evaluate(n.match_close);if(typeof e!="number")continue;const f=typeof n.tolerance=="number"?n.tolerance:5;if(!p(l,e)&&Math.abs(l-e)<=f)return n}catch{}return null}export{E as detectErrorPattern};
