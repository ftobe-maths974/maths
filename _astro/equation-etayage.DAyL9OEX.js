import{l as y}from"./rapidos-visuals-integration.B_AgYn_F.js";import"./editor.CJZspgfY.js";const T="content",u=(r,e)=>Math.floor(Math.random()*(e-r+1))+r,m=(r,e)=>{let t;do t=u(r,e);while(t===0);return t};function F(r){const e=r&&r.form||"ax+b=c";if(e==="ax=b"){const i=u(2,7),o=u(2,9);return{...r,equation:`${i}x = ${i*o}`,_seed:Math.random()}}if(e==="x+a=b"){const i=u(1,9),o=u(2,9);return{...r,equation:`x + ${i} = ${i+o}`,_seed:Math.random()}}if(e==="x+a=b-relatif"){let i,o,s;do i=m(-9,9),o=m(-9,9),s=o+i;while(s===0||Math.abs(s)>12);const l=i<0?` - ${-i}`:` + ${i}`;return{...r,equation:`x${l} = ${s}`,_seed:Math.random()}}if(e==="ax=b-relatif"){let i,o;do i=u(-7,7),o=m(-9,9);while(Math.abs(i)<2);const s=i*o,l=i<0?`-${-i}`:`${i}`;return{...r,equation:`${l}x = ${s}`,_seed:Math.random()}}if(e==="ax+b=cx+d"){let i,o,s,l,d;do i=u(3,7),o=u(1,i-1),s=u(2,6),l=u(1,12),d=l-s*(i-o);while(d<=0||d>12);return{...r,equation:`${i}x + ${d} = ${o}x + ${l}`,_seed:Math.random()}}if(e==="ax+b=cx+d-relatif"){let i,o,s,l,d;do i=m(-5,5),o=m(-4,4),s=m(-6,6),l=u(-8,8),d=l-s*(i-o);while(i===o||Math.abs(d)>15);const f=i<0?`-${-i}`:`${i}`,h=o<0?`-${-o}`:`${o}`,p=d<0?` - ${-d}`:` + ${d}`,b=l<0?` - ${-l}`:` + ${l}`;return{...r,equation:`${f}x${p} = ${h}x${b}`,_seed:Math.random()}}const t=u(2,7),n=u(2,7),a=u(1,9),c=t*n+a;return{...r,equation:`${t}x + ${a} = ${c}`,_seed:Math.random()}}const I=`
.ee-wrap {
  padding: 4px 8px;
  margin: 0 auto;
