const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["_astro/rapido-engine.Cn-x22e5.js","_astro/preload-helper.CLcXU_4U.js","_astro/rapidos-visuals-integration.BI7FvBJX.js","_astro/js-yaml.CwjAzRNl.js","_astro/editor.Di92v2Ao.js","_astro/error-detector.Cylz1YnA.js"])))=>i.map(i=>d[i]);
import{_ as x}from"./preload-helper.CLcXU_4U.js";import $ from"./js-yaml.CwjAzRNl.js";import{T as A}from"./rapido-engine.Cn-x22e5.js";import"./rapidos-visuals-integration.BI7FvBJX.js";import"./editor.Di92v2Ao.js";const L=`# Matrice d'indices — fiche 24.04 retour à l'unité (cycle 3)
#
...
`,E=`# Matrice d'indices — Zefor 02-glisse-nombre (×/÷ 10 ; 100 ; 1000)
#
# BROUILLON v0 — à relire/corriger par Florian (style, formulation, pédago).
#
# Le visuel glisse-nombre-zefor expose 3 phases via getCurrentPhase() :
#   reponse_directe        — champ libre vide
#   erreur_reponse_directe — réponse fausse SANS pattern reconnu (fallback ;
#                            si un pattern matche, error-detector prend le pas,
#                            cf. 02-glisse-nombre.yaml dans error-patterns/)
#   done                   — réponse correcte
#
# Variables dispo dans les templates (NOMBRES uniquement — les chaînes comme
# \`op\` ne sont PAS injectées par le moteur d'indices) :
#   [n]      nombre de départ
#   [k]      facteur (10 / 100 / 1000)
#   [p]      nombre de rangs de glissement (= log10 k : 1 / 2 / 3)
#   [answer] résultat correct (à NE PAS donner en clair dans n1/n2)
#
# Niveaux n1 → n2 → n3 : reformulation → pointage → décomposition (sans le
# résultat numérique). On ne dit jamais « la virgule se déplace ».
#
# Les 5 variantes partagent la même matrice (alias YAML *gn).

variantes:

  frag-mult-entiers: &gn

    phases:

      reponse_directe:
        description: Champ vide, l'élève n'a pas encore répondu
        n1: "Regarde l'opération affichée. Avec le glisse-nombre, **ce sont les chiffres qui glissent**, pas la virgule. Par [k], chaque chiffre prend une valeur [k] fois plus grande (×) ou plus petite (÷)."
        n2: "Compte les rangs : par 10 → 1 rang, par 100 → 2 rangs, par 1000 → 3 rangs. Ici, tu glisses de **[p] rang(s)** — vers la **gauche** si tu multiplies, vers la **droite** si tu divises."
        n3: "Ouvre le glisse-nombre (💡) et clique sur « Faire glisser » : tu verras chaque chiffre changer de rang, et les 0 se placer jusqu'aux unités."

      erreur_reponse_directe:
        description: Réponse incorrecte, aucun pattern d'erreur reconnu
        n1: "Ce n'est pas tout à fait ça. Reprends avec le glisse-nombre : la virgule reste **fixe**, ce sont les **chiffres** qui glissent de [p] rang(s)."
        n2: "Vérifie deux choses : le **sens** (× rend plus grand, ÷ rend plus petit) et le **nombre de rangs** ([p] pour [k])."
        n3: "Ouvre le glisse-nombre (💡), clique « Faire glisser » puis lis le résultat rang par rang."

  satisf-xdiv-entiers: *gn
  satisf-mult-decimaux: *gn
  ts-decimaux-complexes: *gn
  expert-mult-petit: *gn
`,y=`# Matrice d'indices — fiche Zefor 2 graduer-axe-fraction (cycle 3, GS 2.5)
#
...
`,C=`# Indices Zefor 04-tableur (fiche unique tableur)
#
...
#   3. DNB — fragile-somme, satisf-moyenne, ts-proportionnalite, expert-proportion-mixte
...
variantes:
  ...
  tableur-c2-seul:
    patterns_formule:
      - id: trop_de_dollars
        formula_regex: "\\\\$.*\\\\$"
        indice: "Tu as mis **trop de $**. Comme on recopie ta formule vers le bas (même colonne B), il suffit de **bloquer la ligne** de B1 avec **un seul $** : \`B$1\` (le $ devant le chiffre uniquement)."
        relance: "Remplace \`$B$1\` par \`B$1\` (un seul $, devant le 1)."
  ...
  # Proportionnalité — fonction linéaire y = k·x (produit en croix).
  # (ex-ts-fonction-affine, refondue en tableau de proportionnalité.)
  ts-proportionnalite:
    phases:
      cellule_vide:
        n1: "C'est une fonction **linéaire** : $y = k\\\\times x$. Toutes les colonnes utilisent le **même coefficient** $k$."
        n2: "Trouve d'abord $k$ avec la colonne où $x$ ET $y$ sont connus : $k = y \\\\div x$. Repère le **1** : à $x=1$, $y$ vaut justement $k$."
        n3: "Une fois $k$ connu, chaque case se complète : $y = k\\\\times x$ (et $x = y \\\\div k$)."
      erreur_calcul:
        n1: "Vérifie que tu utilises bien le **produit en croix** et pas une addition."
        n2: "Le rapport $y \\\\div x$ doit être **identique** dans toutes les colonnes (c'est $k$)."
        n3: "Reprends la colonne connue pour retrouver $k$, puis applique $y = k\\\\times x$."
`,R=`# Matrice d'indices — Zefor 03-volume-cubes (calcul de volume, cubes de 1 cm³)
#
# BROUILLON v0 — à relire/corriger par Florian.
#
# Phases du visuel volume-cubes-zefor (getCurrentPhase) :
#   reponse_directe        — champ vide
#   erreur_reponse_directe — réponse fausse sans pattern reconnu (fallback)
#   done                   — réponse correcte
#
# Variables (NOMBRES seulement) : [lx] [ly] [lz] [answer] (answer à NE PAS donner).
# Voix socratique ; n1 reformule, n2 pointe les couches, n3 décompose
# (longueur × largeur × hauteur) sans donner le total. Les 4 variantes partagent
# la matrice (alias YAML *vc).
...
variantes:
  ...
  frag-compter: &vc
    ...
    phases:
      ...
      reponse_directe:
        description: Champ vide, l'élève n'a pas répondu
        n1: "Le solide est fait de petits cubes de **1 cm³**. Tu peux les compter un par un… mais il y a plus rapide, et tu n'oublieras pas les cubes **cachés**."
        n2: "Compte les cubes d'**une seule couche** (le sol) : **[lx] × [ly]**. Puis regarde combien il y a d'**étages**."
        n3: "Volume = pavé plein **[lx] × [ly] × [lz]** (moins les cubes enlevés s'il en manque). Ouvre 💡 « Découper en couches »."

      erreur_reponse_directe:
        description: Réponse incorrecte, aucun pattern reconnu
        n1: "Ce n'est pas le bon compte. As-tu pensé aux cubes **cachés** (au milieu, derrière, dessous) ? Ils comptent aussi."
        n2: "Compte par **couches** : une couche = **[lx] × [ly]** cubes, et il y a **[lz]** couches."
        n3: "Volume = **[lx] × [ly] × [lz]** cm³ (moins les cubes enlevés s'il en manque). 💡 pour découper en couches."

  satisf-pave: *vc
  expert-creuse: *vc
`,M=`# Matrice d'indices — Zefor 04-scratch-poly (QCM construction polygone régulier)
#
# BROUILLON v0 — à relire/corriger par Florian.
#
# Phases (getCurrentPhase) : reponse_directe / erreur_reponse_directe / done.
# Variables (nombres) : [n] (côtés), [ang] (= 360/n), [side].
# Le piège central = l'angle de rotation. Indices : faire émerger « angle = 360 ÷ n ».
...
variantes:
  ...
  poly-satisf: &poly
    ...
    phases:
      ...
      reponse_directe:
        description: Aucun programme choisi
        n1: "Pour tracer un polygone régulier, le lutin répète **[n] fois** la même chose : avancer d'un côté, puis **tourner**. Toute la question est : tourner de **combien** ?"
        n2: "À chaque sommet, le lutin fait un petit tour. Après avoir fait le tour complet du polygone, il a tourné en tout d'**un tour entier = 360°**, réparti sur **[n]** sommets."
        n3: "Donc l'angle à chaque sommet = **360 ÷ [n]**. Cherche le programme avec **répéter [n]** et **tourner de [ang]°**."

      erreur_reponse_directe:
        description: Mauvais programme choisi
        n1: "Vérifie deux choses : le **nombre de répétitions** (autant que de côtés = [n]) et surtout l'**angle de rotation**."
        n2: "L'angle, ce n'est pas le nombre de côtés ni l'angle « dans » le polygone : c'est de combien le lutin **pivote** à chaque sommet. Les [n] rotations doivent faire **360° en tout**."
        n3: "Angle = 360 ÷ [n] = **[ang]°**. Le bon programme : **répéter [n]** fois, **tourner de [ang]°**."

  poly-ts: *poly
`,T=`# Matrice d'indices — fiche 24.04 retour à l'unité (cycle 3)
#
# BROUILLON v0 — à relire/corriger par Florian (style, formulation, pédago).
#
# Format :
  ...
  satisf-schema-entiers:
    ...
    erreurs_types:
      ...
      - id: dissymetrie_facteurs
        ...
        cible_phase: div_droite|mult_droite
        indice: "Dans un tableau de proportionnalité, **le même facteur** apparaît à gauche et à droite. Vérifie."
`,h=Object.assign({"../data/indices/01-retour-unite.yaml":L,"../data/indices/02-glisse-nombre.yaml":E,"../data/indices/02-graduer-axe-fraction.yaml":y,"../data/indices/03-tableur.yaml":C,"../data/indices/03-volume-cubes.yaml":R,"../data/indices/04-scratch-poly.yaml":M,"../data/indices/24.04-retour-unite.yaml":T}),P=(()=>{const e={};for(const i of Object.keys(h)){const a=i.match(/\/([^/]+)\.yaml$/)?.[1];if(a)try{e[a]=$.load(h[i])}catch(t){console.error(`[indices] erreur de parsing ${i}:`,t)}}return e})();function q(e){return e?e.split("/").pop():""}function B(e,i){const a=P[q(e)];return a?.variantes?a.variantes[i]??null:null}function j(e){const i=e?.dataset?.ref;if(!i)return!1;const t=e.querySelector(".variant-content.active")?.dataset?.id;return t?!!B(i,t):!1}function O(e){for(const i of e.querySelectorAll("*"))if(typeof i.getCurrentPhase=="function")return i;return null}async function g(e,i){if(!e)return"";const a=new A;a.reset();const t=r=>/^[a-zA-Z_$][\w$]*$/.test(r);Object.entries(i).forEach(([r,n])=>{(typeof n=="number"||n&&typeof n=="object"&&!Array.isArray(n)||typeof n=="string"&&t(r))&&a.variables.set(r,n)});const p=a.parse(e,"web"),{miniMd:u}=await x(async()=>{const{miniMd:r}=await import("./rapido-engine.Cn-x22e5.js").then(n=>n.k);return{miniMd:r}},__vite__mapDeps([0,1,2,3,4]));return u(p)}async function z(e,i={level:1}){const a=e?.dataset?.ref;if(!a)return null;const t=e.querySelector(".variant-content.active"),p=t?.dataset?.id;if(!p)return null;const u=B(a,p),r=O(e),n=r?.getCurrentPhase?.()??"initial";if(n==="done")return{html:"Tu as déjà tout bon ici. Bravo !",phase:n,level:1,isDone:!0};const d=t?.visualData?.config??{},o=Math.min(Math.max(i.level|0,1),3);if(n==="erreur_reponse_directe")try{const{detectErrorPattern:l}=await x(async()=>{const{detectErrorPattern:f}=await import("./error-detector.Cylz1YnA.js");return{detectErrorPattern:f}},__vite__mapDeps([5,3,0,1,2,4])),m=r?.querySelector?.(".rapido-input")?.value??"",c=l({ref:q(a),variantId:p,studentValue:m,vars:d});if(c){const b=o===2&&c.relance||c.indice;return{html:await g(b,d),phase:n,level:o,isDone:!1,patternId:c.id}}}catch(l){console.error("[indices] error-detector:",l)}if(u?.patterns_formule&&typeof r?.getCurrentStudentValue=="function")try{const l=r.getCurrentStudentValue()||"";for(const s of u.patterns_formule){if(!s.formula_regex)continue;let m;try{m=new RegExp(s.formula_regex,s.flags??"i")}catch{continue}if(m.test(l)){const f=o===2&&s.relance||s.indice;return{html:await g(f,d),phase:n,level:o,isDone:!1,patternId:s.id}}}}catch(l){console.error("[indices] patterns_formule:",l)}if(!u)return null;const v=u.phases?.[n];if(!v)return null;const _=v[`n${o}`];return _?{html:await g(_,d),phase:n,level:o,isDone:!1}:null}export{j as cardHasIndices,z as getNextIndice};
