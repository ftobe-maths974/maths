import{ensureSharedStyles as v}from"./vis-input.Cu4TzLbY.js";import"./editor.CJZspgfY.js";import"./rapido-engine.DOqdhf1g.js";import"./rapidos-visuals-integration.CJdE1YfW.js";import"./js-yaml.CwjAzRNl.js";const A="content",L=!0,f={control:"#FFAB19",motion:"#4C97FF"};function y(p,e,i){const r=s=>`<span class="sp-num">${s}</span>`,t=`<div class="sp-block spz-motion">avancer de ${r(i)} pas</div><div class="sp-block spz-motion">tourner ↻ de ${r(e)} degrés</div>`;return`<div class="sp-prog spz-prog"><div class="sp-block spz-hat">quand 🏁 cliqué</div><div class="sp-loop"><div class="sp-loop-top">répéter ${r(p)} fois</div><div class="sp-loop-mid"><div class="sp-loop-bar"></div><div class="sp-loop-body">${t}</div></div><div class="sp-loop-bot"></div></div></div>`}function z(p){const t=[];for(let s=0;s<p;s++){const a=-Math.PI/2+2*Math.PI*s/p;t.push(`${(46+38*Math.cos(a)).toFixed(1)},${(46+38*Math.sin(a)).toFixed(1)}`)}return`<svg viewBox="0 0 92 92" width="92" height="92" aria-label="polygone régulier à ${p} côtés">
    <polygon points="${t.join(" ")}" fill="none" stroke="#0369a1" stroke-width="3" stroke-linejoin="round"/></svg>`}const k={3:"triangle équilatéral",4:"carré",5:"pentagone régulier",6:"hexagone régulier",7:"heptagone régulier",8:"octogone régulier",9:"ennéagone régulier",10:"décagone régulier"},w=`
...
.spz-opts { display:flex; gap:14px; flex-wrap:wrap; align-items:flex-start; justify-content:center; }
.spz-opt {
...
`;function $(){if(document.getElementById("spz-styles"))return;const p=document.createElement("style");p.id="spz-styles",p.textContent=w,document.head.appendChild(p)}let S=0;class _ extends HTMLElement{static get observedAttributes(){return["n","ang","side","options","correct"]}connectedCallback(){v(),$(),this.dataset.placeMode="1",this._render()}attributeChangedCallback(){this.isConnected&&this._render()}_cfg(){const e=parseInt(this.getAttribute("n"),10),i=parseInt(this.getAttribute("ang"),10),r=parseInt(this.getAttribute("side"),10);let t=[];try{t=JSON.parse(this.getAttribute("options")||"[]")}catch{}const s=parseInt(this.getAttribute("correct"),10);return{n:e,ang:i,side:r,options:t,correct:s}}_render(){const{n:e,side:i,options:r,correct:t}=this._cfg();if(!Number.isFinite(e)||!r.length){this.innerHTML="";return}this.innerHTML="";const s=k[e]||`polygone à ${e} côtés`,a=`spz-${++S}`,n=document.createElement("div");n.className="spz-wrap",n.innerHTML=`
