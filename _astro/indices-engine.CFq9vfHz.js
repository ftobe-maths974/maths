const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["_astro/rapido-engine.CPEx6osj.js","_astro/rapidos-visuals-integration.B3VuKKWj.js","_astro/editor.CJZspgfY.js","_astro/error-detector.CGUROvWh.js"])))=>i.map(i=>d[i]);
import{_ as x}from"./editor.CJZspgfY.js";import{T as A,j as E}from"./rapido-engine.CPEx6osj.js";import"./rapidos-visuals-integration.B3VuKKWj.js";const L=`# Matrice d'indices — fiche 24.04 retour à l'unité (cycle 3)
#
...
`,M=`# Matrice d'indices — Zefor 03-tableur-prop (intro coefficient $B$1)
#
# Phases du visuel spreadsheet :
#   cellule_vide       — aucune cellule cible remplie
#   formule_invalide   — saisie ne parse pas (= manquant, parenthèse non fermée…)
#   erreur_calcul      — formule correcte syntaxiquement, mais sémantiquement fausse
#   done               — toutes cellules cibles correctes
#
# Niveaux d'indice (cf. CLAUDE.md §Zefor) :
#   n1 — reformulation, pas de pointage spatial
#   n2 — pointe la zone exacte
#   n3 — décompose mais ne donne pas la solution numérique
#
# Tonalité : socratique + diagnostic explicite d'abord.

# ─────────────────────────────────────────────────────────────────────────────
# patterns_formule (couche 4 bis — regex sur la formule brute saisie)
#
# Testés AVANT la matrice statique n1/n2/n3 (priorité). Si un pattern match,
# son indice (level 1) ou relance (level 2) est servi.
# Sources : Haspekian 2005 + Capponi-Balacheff 1989 + analogie Simard 2012.
# ─────────────────────────────────────────────────────────────────────────────
...
variantes:
  ...
  fragile-c2-seul:
    patterns_formule:
      - id: oubli_egal
        formula_regex: "^[^=]"
        indice: "Je vois ce que tu as fait : tu as écrit une valeur ou une expression, mais une formule de tableur commence **toujours par \`=\`**."
        relance: "Réessaie en commençant par \`=\`. Quelle formule pour calculer le prix de 2 mangues ?"
      ...
      - id: oubli_dollar_recopie
        formula_regex: "^=\\\\s*[A-Z]+\\\\d+\\\\s*\\\\*\\\\s*[A-Z]+\\\\d+\\\\s*$"
        indice: "Bonne approche : tu multiplies bien quantité × prix unitaire. Mais pour pouvoir **recopier** ta formule plus tard, il faut **bloquer** la référence à B1 avec des \`$\`. Sinon en B4, B1 deviendrait B2…"
        relance: "Tape \`=A3*$B$1\` (dollar devant la lettre ET devant le chiffre)."

    phases:
      cellule_vide:
        description: Aucune cellule remplie
        ...
        n3: "Tape : \`=A3*B1\`. Mais pour pouvoir recopier ta formule plus tard, ajoute des **\`$\`** autour de B1 : \`=A3*$B$1\`."

      formule_invalide:
        description: Saisie ne parse pas
      ...
      erreur_calcul:
        description: Formule syntaxiquement OK mais sémantiquement fausse
        ...
        n3: "La formule attendue est \`=A3*$B$1\` (les \`$\` ne changent rien ici, mais préparent la recopie)."

  satisf-drag-fill:
    patterns_formule:
      - id: oubli_egal
        formula_regex: "^[^=]"
        indice: "Une formule commence **toujours par \`=\`**."
        relance: "Réessaie : \`=A3*$B$1\`."
      - id: oubli_dollar_recopie
        formula_regex: "^=\\\\s*[A-Z]+\\\\d+\\\\s*\\\\*\\\\s*[A-Z]+\\\\d+\\\\s*$"
      ...
      - id: additif_au_lieu_de_multiplicatif
        formula_regex: "^=[A-Z]+\\\\d+\\\\s*\\\\+\\\\s*\\\\$?[A-Z]+\\\\$?\\\\d+"
        indice: "Tu as **additionné** au lieu de **multiplier**. Pour un total prix = quantité × prix unitaire, on multiplie."
        relance: "Tape \`=A3*$B$1\`."
      - id: valeur_au_lieu_de_reference
        formula_regex: "^=\\\\s*[\\\\d,.]+\\\\s*$"
        indice: "Tu as tapé une **valeur**, pas une **formule**. L'intérêt d'un tableur, c'est que le calcul se met à jour quand les données changent."
        relance: "Utilise les références : \`=A3*$B$1\`."

    phases:
      cellule_vide:
        description: Pas encore de saisie
        n1: "Commence par compléter **B3** : 3 articles, à raison de B1 € l'un. Quelle formule ?"
      ...
      formule_invalide:
        description: Saisie ne parse pas
        n1: "Vérifie que ta formule commence par \`=\` et que les noms de cellules sont bien écrits (lettre + chiffre)."
        n2: "Pour bloquer une référence, on met \`$\` **avant la lettre ET avant le chiffre** : \`$B$1\`, pas \`B$1\` ou \`$B1\`."
        n3: "La formule pour B3 est \`=A3*$B$1\`."

      erreur_calcul:
        description: Une ou plusieurs cellules sont fausses (probablement les $ manquants)
        n1: "Une des cellules est juste, mais pas les autres. C'est peut-être que ta référence à B1 **se décale** quand tu recopies."
        n2: "Quand tu recopies \`=A3*B1\` vers le bas, la référence \`B1\` devient \`B2\`, puis \`B3\`. Or tu veux que \`B1\` reste toujours \`B1\`."
        n3: "Repasse en B3 et écris \`=A3*$B$1\` (avec les \`$\`). Puis recopie vers le bas : maintenant \`B1\` reste fixe."
  ...
  ts-decimaux:
    phases:
      cellule_vide:
        ...
        n2: "N'oublie pas les \`$\` autour de B1 pour pouvoir recopier la formule en B4, B5, B6."
        n3: "Tape \`=A3*$B$1\` en B3 et recopie."

      formule_invalide:
        description: Saisie ne parse pas
        n1: "Commence par \`=\` et utilise \`*\` pour la multiplication."
        n2: "Pour bloquer B1 : \`$B$1\` (dollar devant la lettre ET devant le chiffre)."
        n3: "Formule en B3 : \`=A3*$B$1\`."

      erreur_calcul:
        description: Erreur sémantique
        n1: "Vérifie : si tu recopies, est-ce que la référence à B1 reste bien à B1 ?"
        n2: "Sinon, ajoute les \`$\` : \`=A3*$B$1\` au lieu de \`=A3*B1\`."
        n3: "Solution : \`=A3*$B$1\` en B3, puis recopier vers le bas."
`,$=`# Matrice d'indices — Zefor 04-tableur-dnb (style DNB 3e)
#
# Phases : cellule_vide | formule_invalide | erreur_calcul | done
# Sources didactiques :
#   - DNB juin 2012 (somme), juin 2013 (affine), juin 2013 Polynésie (ALEA)
#   - Haspekian 2005 (erreurs plage mal bornée, =/+ confusion)
...
variantes:
  ...
  fragile-somme:
    patterns_formule:
      - id: oubli_egal
        formula_regex: "^[^=]"
        indice: "Une formule de tableur commence **toujours par \`=\`**."
        relance: "Tape \`=SOMME(B2:E2)\`."
      - id: somme_sans_plage
  ...
  satisf-moyenne:
    patterns_formule:
      - id: oubli_egal
        formula_regex: "^[^=]"
        indice: "Une formule commence par \`=\`."
        relance: "Tape \`=MOYENNE(B2:F2)\`."
...
`,y=`# Matrice d'indices — fiche 24.04 retour à l'unité (cycle 3)
#
# BROUILLON v0 — à relire/corriger par Florian (style, formulation, pédago).
#
# Format :
  ...
  satisf-schema-entiers:
    ...
    erreurs_types:
      ...
      - id: dissymetrie_facteurs
        ...
        cible_phase: div_droite|mult_droite
        indice: "Dans un tableau de proportionnalité, **le même facteur** apparaît à gauche et à droite. Vérifie."
`,h=Object.assign({"../data/indices/01-retour-unite.yaml":L,"../data/indices/02-graduer-axe-fraction.yaml":C,"../data/indices/03-tableur-prop.yaml":M,"../data/indices/04-tableur-dnb.yaml":$,"../data/indices/24.04-retour-unite.yaml":y}),T=(()=>{const e={};for(const n of Object.keys(h)){const a=n.match(/\/([^/]+)\.yaml$/)?.[1];if(a)try{e[a]=E.load(h[n])}catch(t){console.error(`[indices] erreur de parsing ${n}:`,t)}}return e})();function q(e){return e?e.split("/").pop():""}function B(e,n){const a=T[q(e)];return a?.variantes?a.variantes[n]??null:null}function j(e){const n=e?.dataset?.ref;if(!n)return!1;const t=e.querySelector(".variant-content.active")?.dataset?.id;return t?!!B(n,t):!1}function S(e){for(const n of e.querySelectorAll("*"))if(typeof n.getCurrentPhase=="function")return n;return null}async function _(e,n){if(!e)return"";const a=new A;a.reset(),Object.entries(n).forEach(([r,i])=>{(typeof i=="number"||i&&typeof i=="object"&&!Array.isArray(i))&&a.variables.set(r,i)});const t=a.parse(e,"web"),{miniMd:p}=await x(async()=>{const{miniMd:r}=await import("./rapido-engine.CPEx6osj.js").then(i=>i.f);return{miniMd:r}},__vite__mapDeps([0,1,2]));return p(t)}async function V(e,n={level:1}){const a=e?.dataset?.ref;if(!a)return null;const t=e.querySelector(".variant-content.active"),p=t?.dataset?.id;if(!p)return null;const r=B(a,p),i=S(e),u=i?.getCurrentPhase?.()??"initial";if(u==="done")return{html:"Tu as déjà tout bon ici. Bravo !",phase:u,level:1,isDone:!0};const d=t?.visualData?.config??{},c=Math.min(Math.max(n.level|0,1),3);if(u==="erreur_reponse_directe")try{const{detectErrorPattern:l}=await x(async()=>{const{detectErrorPattern:f}=await import("./error-detector.CGUROvWh.js");return{detectErrorPattern:f}},__vite__mapDeps([3,0,1,2])),m=i?.querySelector?.(".rapido-input")?.value??"",o=l({ref:q(a),variantId:p,studentValue:m,vars:d});if(o){const b=c===2&&o.relance||o.indice;return{html:await _(b,d),phase:u,level:c,isDone:!1,patternId:o.id}}}catch(l){console.error("[indices] error-detector:",l)}if(r?.patterns_formule&&typeof i?.getCurrentStudentValue=="function")try{const l=i.getCurrentStudentValue()||"";for(const s of r.patterns_formule){if(!s.formula_regex)continue;let m;try{m=new RegExp(s.formula_regex,s.flags??"i")}catch{continue}if(m.test(l)){const f=c===2&&s.relance||s.indice;return{html:await _(f,d),phase:u,level:c,isDone:!1,patternId:s.id}}}}catch(l){console.error("[indices] patterns_formule:",l)}if(!r)return null;const v=r.phases?.[u];if(!v)return null;const g=v[`n${c}`];return g?{html:await _(g,d),phase:u,level:c,isDone:!1}:null}export{j as cardHasIndices,V as getNextIndice};
